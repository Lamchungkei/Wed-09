...
class GameScene(Scene):
    ...
    def update(self):
        ...
        if self.left_button_down == True:
            spaceshipMove = Action.move_by(-1*self.ship_move_speed, 0.0, 0.1)
            self.spaceship.run_action(spaceshipMove)
            
        if self.right_button_down == True:
            spaceshipMove = Action.move_by(self.ship_move_speed, 0.0, 0.1)
            self.spaceship.run_action(spaceshipMove)
            
        # every update, radomly check if a new alien should be created
        alien_create_chance = random.randint(1, 120)
        if alien_create_chance <= self.alien_attack_rate:
            self.add_alien()
            
        # check every update if a missle is off screen
        for missle in self.missiles:
            if missle.position.y > self.size_of_screen.y - 50:
               missle.remove_from_parent()
               self.missiles.remove(missle)
